...
## 7. Open Research Areas — Queue for Deep Dive

These are the areas that still need dedicated research passes. The six open questions from before are preserved here alongside the new ones identified from this session.

### Carry-over Open Questions
1. **ISP line decision** — Shared campus line vs. dedicated SSC line. Needs real conversation with IT program head, but research should surface what other campus PisoWifi deployments typically do.
2. **openNDS on Ubuntu 24.04 compatibility** — Verify in documentation and community reports before committing to it as the captive portal daemon.
3. **OPNsense vs OpenWrt x86 in Proxmox** — Beginner-focused comparison. Which has better community support for captive portal + openNDS use case specifically?
4. **Coolify + Docker networking in a Proxmox VM** — Does Coolify's internal Docker management create iptables conflicts with the OPNsense routing VM? Research known issues.
5. **Campus policy on student org monetary collection** — What exact paperwork does ISPSC or a typical Philippine SUC require? This shapes the app authorization flow design.
6. **CHED CMO 20 pitch strategy** — How to frame the 25% admin share as a benefit to administration, not a cost to the student org.

### New Research Areas (from this session)

7. **Feature parity benchmarking against real PisoWifi software** — Research WiFi5soft, AdoPiSoft, PisoFi, and any other widely-deployed Philippine PisoWifi platforms. Catalog every user-facing and admin-facing feature they offer. This becomes the feature checklist StudentHub Phase 1 is measured against. Look specifically at: session UI flows, pause/resume UX, voucher systems, top-up methods, admin dashboards, reporting, and hardware compatibility (bill acceptors, coin acceptors, power relays).

8. **Bill acceptor integration** — The research so far only covers coin acceptors + ESP32. Real PisoWifi deployments often support bill acceptors (₱20, ₱50, ₱100 notes) as well. Research how bill acceptors interface with microcontrollers — pulse output vs. serial, voltage levels, popular models available in the Philippine market, and how existing open-source projects wire them up.

9. **Power relay / vending unit power management** — The physical coin-top-up unit (the box students walk up to with the coin slot and indicator lights) needs to be power-efficient, since it runs continuously on campus. A relay allows the server to remotely switch power to the unit's components — coin acceptor, LED indicators, display — on and off on a schedule or on demand (e.g., off during non-school hours, on during operating hours). Research how existing PisoWifi deployments wire relays for this purpose: what relay modules are used, how the ESP32 drives them, and how the server-side schedule or trigger is managed.

10. **MAC randomization — evidence from real deployments** — Go beyond the theoretical architecture already researched. Find evidence of how WiFi5soft, AdoPiSoft, JuanFi, or other open-source PisoWifi projects actually handle MAC randomization in practice. Do they use browser tokens? Persistent SSIDs? WPA2 pre-shared keys to force stable MACs? What do their GitHub issues and community forums say about this problem in the field?

11. **App access tier management — how to implement without manual iptables** — Research how to build the admin-configurable per-app access tier (free / credit-gated / monetization-authorized) in a way that doesn't require manual firewall rule edits each time. Options: reverse proxy rules managed by Laravel, Coolify labels/tags, or a custom middleware layer. Find precedent in existing open-source PaaS management tools.

12. **Proxmox on a single-drive Windows machine** — Concrete guide for dual-booting or dedicating a second drive to Proxmox on a consumer Windows PC for development use. Verify this is stable enough as a dev environment (not just a live USB).
